...
@section('content')
    <div class="card">
        <div class="card-header">
            @canany(['peminjamanBaruTools.create', 'peminjamanBaruTools.store'])
                        ...
                        @foreach ($data_peminjaman as $peminjaman)
                            ...
                            <tr>
                                ...
                                <td class="align-middle text-center">
                                    @can('peminjamanBaruTools.review')
                                        @if($peminjaman->approval_peminjaman === null)
                                            <a href="{{ route('peminjamanBaruTools.review', $peminjaman->id) }}" class="mx-2 my-1 btn btn-info">Review</a>
                                        @endif
                                    @endcan
                                    @can('peminjamanBaruTools.del')
                                        @if ($peminjaman->id_user == auth()->user()->id)
                                            <a href="{{ route('peminjamanBaruTools.del', $peminjaman->id) }}" class="mx-2 my-1 {{ $peminjaman->approval_peminjaman === null ? 'btn btn-warning' : 'btn btn-danger'}}" data-confirm-delete="true">{{ $peminjaman->approval_peminjaman === null ? 'Batalkan' : 'Hapus'}}</a>
                                        @endif
                                    @endcan
                                </td>
                            </tr>
        ...
        <div class="modal fade" id="modalCreate" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action={{ route('peminjamanBaruTools.create') }} method="POST">
                        {{ csrf_field() }}
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                ...
                                <span class="text-info">*bisa mengisikan lebih dari 1 tools</span>
                                @error('tools')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
